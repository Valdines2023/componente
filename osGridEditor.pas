  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for jCol := 1 to srcGrid.CellCombo[iCol, iRow].ComboGrid.Cols do
          tgtGrid.CellCombo[iCol, iRow].ComboGrid.Col[jCol].Assign(srcGrid.CellCombo[iCol, iRow].ComboGrid.Col[jCol]);
        if srcGrid.CellCombo[iCol, iRow].ComboGrid.StoreData then
           for dataRow := 1 to srcGrid.CellCombo[iCol, iRow].ComboGrid.Rows do
              for dataCol := 1 to srcGrid.CellCombo[iCol, iRow].ComboGrid.Cols do
                tgtGrid.CellCombo[iCol, iRow].ComboGrid.Cell[dataCol, dataRow] := srcGrid.CellCombo[iCol, iRow].ComboGrid.Cell[dataCol, dataRow];
      end;
    end;
  end;
end;
...
begin
  if TestGridColorProperty(sPropName) then
     ...
     Result := ptBoolean
  else if PropType(SelectedCombo.ComboGrid, sPropName) = tkEnumeration then
     Result := ptCombo
  else if PropType(SelectedCombo.ComboGrid, sPropName) = tkClass then
     Result := ptButton
  else if PropType(SelectedCombo.ComboGrid, sPropName) = tkInteger then
     Result := ptSpin
  else
     Result := ptEdit;
end;
...
begin
  Result := False;
  if PropType(SelectedCombo.ComboGrid, PropertyName) = tkEnumeration then
  begin
    sValue := GetEnumProp(SelectedCombo.ComboGrid, PropertyName);
    Result := (LowerCase(sValue) = 'true') or (Lowercase(sValue) = 'false');
  end;
end;
...
procedure TfmOsGridEditor.veEditorComboInit(Sender: TObject; Combo: TtsComboGrid;
  DataCol, DataRow: Integer);
begin
  Combo.Width := veEditor.Col[2].Width - 15;
...
procedure TfmOsGridEditor.veColEditorComboInit(Sender: TObject;
  Combo: TtsComboGrid; DataCol, DataRow: Integer);
begin
  Combo.Width := veColEditor.Col[2].Width - 15;
  Combo.Color := clWindow;
  Combo.DropDownStyle := ddDropDownList;
  Combo.GridLines := glNone;
  Combo.AutoSearch := asTop;
...
procedure TfmOsGridEditor.sampleGridColResized(Sender: TObject;
  RowColnr: Integer);
begin
  if (RowColnr <= 0) or FBinding or (sampleGrid.ResizingColumns) then exit;
...
begin
  if (RowColnr <= 0) or FBinding or (sampleDbGrid.ResizingColumns) then exit;
...
begin
  ...
  try
    ...
    chComboStoreData.Checked := SelectedCombo.ComboGrid.StoreData;
    ...
    if (SelectedCombo is TtsDbCombo) and
       (TtsDbCombo(SelectedCombo).DataSource <> Nil) then
    begin
       sComboFullName := GetFullName(TtsDbCombo(SelectedCombo).DataSource);
       ...
       LoadComboDataBookmarks(TtsDbCombo(SelectedCombo).DataSource.DataSet);
    end
...
begin
  ...
  begin
     ...
     SelectedCombo.ComboGrid.Cols := SelectedCombo.DropDownCols;
     ResetComboGrid;
     if (SelectedCombo is TtsDbCombo) and
        (TtsDbCombo(SelectedCombo).DataSource <> Nil) and
        (TtsDbCombo(SelectedCombo).DataSource.DataSet.FieldCount >= StrToInt(edDropDownCols.Text)) then
     begin
       SelectedCombo.ComboGrid.Col[SelectedCombo.DropDownCols].FieldName :=
           TtsDbCombo(SelectedCombo).DataSource.DataSet.Fields.Fields[SelectedCombo.DropDownCols].FieldName;
       ResetComboColumns;
     end;
     ResetComboColProperties;
     udValueCol.Max := SelectedCombo.DropDownCols;
...
procedure TfmOsGridEditor.ResetComboGrid;
  ...
  begin
    ...
    gdComboData.StoreData := SelectedCombo.ComboGrid.StoreData;
  end;
  ...
  begin
    ...
    try
      for i := 1 to gdComboData.Cols do
      begin
        if i <= SelectedCombo.ComboGrid.Cols then
        begin
          if SelectedCombo is TtsDbCombo then
             theCol := TtsDbCombo(SelectedCombo).ComboGrid.Col[TtsDbCombo(SelectedCombo).ComboGrid.DataColnr[i]]
          else
             theCol := TtsCombo(SelectedCombo).ComboGrid.Col[i];
  ...
  begin
    ...
    begin
      if i <= SelectedCombo.ComboGrid.Cols then
         gdComboData.Col[i].Width := TtsCombo(SelectedCombo).ComboGrid.Col[i].Width;
    end;
  end;
  ...
  var i, j : Integer;
  begin
    for i := 1 to SelectedCombo.ComboGrid.Rows do
       for j := 1 to SelectedCombo.DropDownCols do
         gdComboData.Cell[j, i] := SelectedCombo.ComboGrid.Cell[SelectedCombo.ComboGrid.DataColnr[j], i];
  end;
begin
  if SelectedCombo = Nil then
  begin
  ...
  begin
    gdComboData.DefaultColWidth := SelectedCombo.ComboGrid.DefaultColWidth;
    if SelectedCombo is TtsDbCombo then
       TtsCustomDBGrid_(SelectedCombo.ComboGrid).FieldState := fsCustomized;
    ResetGrid;
    if gdComboData.StoreData then
    begin
       gdComboData.GridMode := gmEditInsert;
       gdComboData.Rows := SelectedCombo.ComboGrid.Rows;
    end
    ...
    try
      ...
      if (SelectedCombo is TtsDbCombo) then
      begin
        if (TtsDbCombo(SelectedCombo).DataSource <> Nil) then
        begin
          gdComboData.Rows := 0;
          if TtsDbCombo(SelectedCombo).DataSource.DataSet.Active then
          begin
            gdComboData.Rows := TtsDbCombo(SelectedCombo).DataSource.DataSet.RecordCount;
    ...
    try
      gdComboData.Font.Assign(SelectedCombo.ComboGrid.Font);
      gdComboData.Color := SelectedCombo.ComboGrid.Color;
      gdComboData.SelectionColor := SelectedCombo.ComboGrid.SelectionColor;
      gdComboData.SelectionFontColor := SelectedCombo.ComboGrid.SelectionFontColor;
      gdComboData.DefaultRowHeight := SelectedCombo.ComboGrid.DefaultRowHeight;
      gdComboData.GridLines := SelectedCombo.ComboGrid.GridLines;
      gdComboData.HorzAlignment := SelectedCombo.ComboGrid.HorzAlignment;
      gdComboData.VertAlignment := SelectedCombo.ComboGrid.VertAlignment;
      gdComboData.Is3D := SelectedCombo.ComboGrid.Is3D;
      gdComboData.LineColor := SelectedCombo.ComboGrid.LineColor;
      gdComboData.WordWrap := SelectedCombo.ComboGrid.WordWrap;
      if gdComboData.StoreData then
         ResetComboData;
    finally
...
begin
  if (not FBinding) and (SelectedCombo <> Nil) then
  begin
    SelectedCombo.ComboGrid.StoreData := chComboStoreData.Checked;
    if chComboStoreData.Checked then
    begin
      cbComboDataSource.ItemIndex := -1;
      if demoGrid is TtsDbGrid then
         TtsDbCombo(SelectedCombo).DataSource := Nil;
    end;
...
begin
  ...
  begin
    try
      TtsDbCombo(SelectedCombo).DataSource := TDataSource(cbComboDataSource.Items.Objects[cbComboDataSource.ItemIndex]);
      SelectedCombo.ComboGrid.Cols := 0;
    except on E:Exception do
...
begin
  if (SelectedCombo = Nil) or
     (SelectedCombo.ComboGrid.Cols < DataCol) or
     (SelectedCombo.ComboGrid.StoreData) then exit;

  try
    theCol := TtsDbCombo(SelectedCombo).ComboGrid.Col[DataCol];
    if (SelectedCombo is TtsDbCombo) and
       (TtsDbCombo(SelectedCombo).DataSource <> Nil) then
    begin
      TtsDbCombo(SelectedCombo).DataSource.DataSet.Bookmark := comboDataBookmarks.Strings[DataRow-1];
      if (DataCol <= SelectedCombo.ComboGrid.Cols) and
         (TtsDbCombo(SelectedCombo).ComboGrid.Col[DataCol] <> Nil) and
         (TtsDbCombo(SelectedCombo).ComboGrid.Col[DataCol].FieldName <> '') then
         Value := TtsDbCombo(SelectedCombo).DataSource.DataSet.FieldByName(TtsDbCombo(SelectedCombo).ComboGrid.Col[DataCol].FieldName).AsString;
    end
    ...
    begin
      ...
      Value := ComboLookupDataSet.FieldByName(TtsDbCombo(SelectedCombo).ComboGrid.Col[DataCol].FieldName).AsString;
    end;
  except on E:Exception do
    begin
      gdComboData.Rows := 0;
...
begin
  ...
  begin
    SelectedCombo.ComboGrid.Col[RowColnr].Width := gdComboData.Col[RowColnr].Width;
    GridModified;
    FColumnsChanged := True;
  end;
end;
...
begin
  ...
  PropertyCount := GetPropList(SelectedCombo.ComboGrid.ClassInfo, tkProperties, @PropList) ;
  for PropertyIndex := 0 to PropertyCount -1 do
  begin
    sName := PropList[PropertyIndex].Name;
...
begin
  Result := VarToStr(GetPropValue(SelectedCombo.ComboGrid, PropertyName, False));
  case GetComboPropertyType(PropertyName) of
    ptSpin    : veComboProperties.CellButtonType[2, veComboProperties.Rows] := btVertSpin;
    ptCombo   :
        begin
          Result := GetEnumProp(SelectedCombo.ComboGrid, PropertyName);
...
begin
  ...
  begin
     SelectedCombo.ComboGrid.ParentFont := False;
     dgFont.Font := SelectedCombo.ComboGrid.Font;
  end
  else if (AnsiCompareText(veComboProperties.Cell[1, DataRow], 'HeadingFont') = 0) then
  begin
     SelectedCombo.ComboGrid.HeadingParentFont := False;
     dgFont.Font := SelectedCombo.ComboGrid.HeadingFont;
  end
  else
     exit;
  if dgFont.Execute then
  begin
    if (AnsiCompareText(veComboProperties.Cell[1, DataRow], 'Font') = 0) then
       SelectedCombo.ComboGrid.Font := dgFont.Font
    else if (AnsiCompareText(veComboProperties.Cell[1, DataRow], 'HeadingFont') = 0) then
       SelectedCombo.ComboGrid.HeadingFont := dgFont.Font;
    ResetComboGrid;
  end;
end;
...
begin
  ...
  if GetComboPropertyType(veComboProperties.Cell[1, DataRow]) = ptColor then
     SetPropertyValueText(SelectedCombo.ComboGrid, veComboProperties.Cell[1, DataRow], StringToColor(FullColorName(veComboProperties.Cell[2, DataRow])))
  else
     SetPropertyValueText(SelectedCombo.ComboGrid, veComboProperties.Cell[1, DataRow], veComboProperties.Cell[2, DataRow]);
  FColumnsChanged := True;
  ResetComboGrid;
end;
...
begin
  ...
  begin
   ...
   begin
     Combo.Rows := LoadEnumList(SelectedCombo.ComboGrid, sPropName);
     if Combo.Rows <= 8 then
        Combo.DropDownRows := Combo.Rows
     else
        Combo.DropDownRows := 8;
     sValue := GetEnumProp(SelectedCombo.ComboGrid, sPropName);
     Combo.CurrentDataRow := EnumList.IndexOf(sValue) + 1;
     Combo.Tag := 3;
   end;
...
procedure TfmOsGridEditor.veComboPropertiesComboInit(Sender: TObject;
  Combo: TtsComboGrid; DataCol, DataRow: Integer);
begin
  Combo.Width := veComboProperties.Col[2].Width - 15;
  Combo.Color := clWindow;
  Combo.DropDownStyle := ddDropDownList;
  Combo.GridLines := glNone;
end;
...
begin
  ...
  if (SelectedCombo <> Nil) then
  begin
    for i := 1 to SelectedCombo.ComboGrid.Cols do
      if (SelectedCombo.ComboGrid.Col[i].FieldName <> '') then
         lbComboFields.Items.Add(SelectedCombo.ComboGrid.Col[i].FieldName);

    if (cbComboDataSource.ItemIndex >= 0) and
       (TtsDbCombo(SelectedCombo).DataSource.DataSet <> Nil) then
    begin
       for i := 0 to TtsDbCombo(SelectedCombo).DataSource.DataSet.FieldCount - 1 do
          if lbComboFields.Items.IndexOf(TtsDbCombo(SelectedCombo).DataSource.DataSet.Fields.Fields[i].FieldName) = -1 then
            lbAllComboFields.Items.Add(TtsDbCombo(SelectedCombo).DataSource.DataSet.Fields.Fields[i].FieldName);
    end
    else if (chAutoLookup.Checked) and
            (ComboLookupDataSet <> Nil) then
    begin
...
begin
  ...
  begin
    ...
    SelectedCombo.ComboGrid.Cols := SelectedCombo.DropDownCols;
    SelectedCombo.ComboGrid.Col[lbComboFields.Items.Count].FieldName := sFldName;
...
begin
  ...
  begin
    ...
    if (SelectedCombo.DropDownCols > 0) and
       (SelectedCombo.ComboGrid.Col[sFldName] <> Nil) then
    begin
      iCol := SelectedCombo.ComboGrid.Col[sFldName].DisplayCol;
      SelectedCombo.ComboGrid.DeleteCols(iCol, iCol);
    end;

    SelectedCombo.ComboGrid.DropDownCols := SelectedCombo.ComboGrid.Cols;
...
procedure TfmOsGridEditor.udComboFieldsClick(Sender: TObject;
  ...
  begin
    for i := 1 to lbComboFields.items.Count do
      SelectedCombo.ComboGrid.Col[i].FieldName := lbComboFields.Items[i-1];
    ResetComboGrid;
  end;
begin
...
begin
  if (SelectedCombo <> Nil) and
     SelectedCombo.ComboGrid.StoreData then
  begin
    if SelectedCombo.ComboGrid.Rows <> gdComboData.Rows then
       SelectedCombo.ComboGrid.Rows := gdComboData.Rows;
    SelectedCombo.ComboGrid.Cell[DataCol, DataRow] := gdComboData.Cell[DataCol, DataRow];
    GridModified;
  end;
end;
...
begin
  ...
  begin
   ...
   begin
     ...
     Combo.CurrentDataRow := EnumList.IndexOf(veCellEditor.Cell[2,DataRow]) + 1;
     Combo.Tag := 3;
   end
  ...
  begin
    ...
    begin
      if SelectedCombo.ComboGrid.Col[i].FieldName = '' then
         ...
         sText := SelectedCombo.ComboGrid.Col[i].FieldName;
      lbColumns2.Items.AddObject(sText, SelectedCombo.ComboGrid.Col[i]);
    end;
...
procedure TfmOsGridEditor.veComboColEditorComboInit(Sender: TObject;
  Combo: TtsComboGrid; DataCol, DataRow: Integer);
begin
  Combo.Width := veComboColEditor.Col[2].Width - 15;
  Combo.Color := clWindow;
  Combo.DropDownStyle := ddDropDownList;
  Combo.GridLines := glNone;
end;
