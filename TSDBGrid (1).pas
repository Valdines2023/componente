...
type
    ...
    protected
        ...
        function  GetDropDownStyle: TtsDropDownStyle; override;
        function  GetComboGrid: TtsDBComboGrid; 
    ...
    public
        ...
        property ComboGrid: TtsDBComboGrid read GetComboGrid;
...
begin
    if Assigned(Combo) then Combo.Datasource := Value;
end;

function TtsDBCol.GetComboDataSource: TDatasource;
begin
    Result := nil;
    if Assigned(Combo) then Result := Combo.Datasource;
end;
...
begin
  ...
  // Converte FromBkm para TBookmarkStr
  SetString(CurBkm, PChar(FromBkm), Length(FromBkm));
  Buffer := FTempBuffer;
  ...
  begin
    ...
    begin
      ...
      CompRes := -1;
      if ToBkm <> nil then
        CompRes := CompareBkm(TBookmark(CurBkm), ToBkm);
      if CompRes <= 0 then
  ...
  // Converte CurBkm de volta para TBookmark
  SetString(FromBkm, PChar(CurBkm), Length(CurBkm));
  Result := (Err <> 0);
  if (Err = DbiErr_Eof) and (ToBkm = '') then
   Found := True;
end;
...
begin
    ...
    begin
        ...
        if (FDataCol >= 1) and (FDataCol <= Grid.Cols) then
            FCombo.ComboGrid.DefaultColWidth := Grid.Col[FDataCol].Width + 10;
    end;
end;
...
begin
    if Value <> ComboGrid.DropDownRows then ComboGrid.DropDownRows := Value;
end;

procedure TtsDBCombo.SetDropDownCols(Value: Longint);
begin
    if Value <> ComboGrid.DropDownCols then ComboGrid.DropDownCols := Value;
end;

procedure TtsDBCombo.SetValueCol(Value: Longint);
begin
    if Value <> ComboGrid.ValueCol then ComboGrid.ValueCol := Value;
end;

procedure TtsDBCombo.SetValueColSorted(Value: Boolean);
begin
    if Value <> ComboGrid.ValueColSorted then ComboGrid.ValueColSorted := Value;
end;

procedure TtsDBCombo.SetCompareType(Value: TtsComboCompareType);
begin
    if Value <> ComboGrid.CompareType then ComboGrid.CompareType := Value;
end;

procedure TtsDBCombo.SetAutoSearch(Value: TtsComboAutoSearchType);
begin
    if Value <> ComboGrid.AutoSearch then ComboGrid.AutoSearch := Value;
end;

procedure TtsDBCombo.SetAutoFill(Value: Boolean);
begin
    if Value <> ComboGrid.AutoFill then ComboGrid.AutoFill := Value;
end;

procedure TtsDBCombo.SetAutoFillConvertCase(Value: TtsConvertCase);
begin
    if Value <> ComboGrid.AutoFillConvertCase then ComboGrid.AutoFillConvertCase := Value;
end;

procedure TtsDBCombo.SetAutoLookup(Value: Boolean);
begin
    if Value <> ComboGrid.AutoLookup then ComboGrid.AutoLookup := Value;
end;

procedure TtsDBCombo.SetDropDownStyle(Value: TtsDropDownStyle);
begin
    if Value <> ComboGrid.DropDownStyle then ComboGrid.DropDownStyle := Value;
end;

procedure TtsDBCombo.SetDataSource(Value : TDataSource);
begin
...
procedure TtsDBCombo.RefreshData;
var i, iRow : Integer;
begin
  if (FDataSource = Nil) or
     (FDataSource.DataSet = Nil) or
     (not FDataSource.DataSet.Active) or
        ...
        try
          ...
          begin
            for i := 1 to ComboGrid.Cols do
              if (ComboGrid.Col[i].FieldName <> '') and
                 (FindField(ComboGrid.Col[i].FieldName) <> Nil) then
                 ComboGrid.Cell[i, iRow] := FieldByName(ComboGrid.Col[i].FieldName).AsString;
            Next;
            Inc(iRow);
          end;
        finally
...
procedure TtsDBCombo.InitCombo;
begin
    {ComboGrid.DataSource := nil;
    ComboGrid.FAutoLookup := True;
    ComboGrid.DatasetType := DatasetType;
    ComboGrid.BookmarkType := BookmarkType;
    ComboGrid.BookmarkCompareType := BookmarkCompareType;
    ComboGrid.RecordSelection := RecordSelection;}
...
//var Field: TtsDBField;
begin
    if not DataBound then begin inherited; Exit; end;
    if Combo.Datasource <> nil then Exit;
    //if Combo.ComboGrid.StoreData then Exit;
    if not Col[CurDataCol].Lookup then Exit;
    if not Combo.AutoLookup then exit;

    //Field := Col[CurDataCol].Field;
    //Combo.ComboGrid.LookupDataset := Field.DatasetField.LookupDataset;

    {if not CellHasCombo then
    begin
        Combo.ComboGrid.FieldState := fsCustomized;
        Combo.ComboGrid.Cols := 0;
        Combo.ComboGrid.InsertCol(1, Field.DatasetField.LookupResultField);
...
begin
    ...
    if Assigned(Combo.ComboGrid.Datasource) then
        Result := Combo.ComboGrid.Datasource <> Datasource;
end;

procedure TtsCustomDBGrid.ComboDropDown(DataCol, DataRow: Longint);
begin
    if not Combo.ComboGrid.DataBound then begin inherited; Exit end;

    if (FDropDownCol = -1) and (FDropDownCol = -1) then
    begin
        if Combo.ComboGrid.Active and CheckInitComboRow then
        with Combo.ComboGrid do
        begin
            MoveFirst;
...
begin
    Value := Unassigned;
    if not Combo.DataBound then begin inherited; Exit end;
    if not Combo.Active then Exit;

    DataColUp := 0;
    CurBkm := Combo.ComboGrid.CurrentDataRow;
    CurRow := Combo.ComboGrid.CurrentDataRow; //FGridData.BufferRow(CurBkm);
    if CurRow <= 0 then
    begin
        Combo.ComboGrid.FGridData.MoveToRecord(CurBkm);
        CurRow := Combo.ComboGrid.FGridData.BufferRow(CurBkm);
    end;

    OldActive := Combo.ComboGrid.FGridData.ActiveRecord;
    try
        Combo.ComboGrid.FGridData.ActiveRecord := CurRow;
        ...
        begin
            ...
            begin
                if Col[CurDataCol].Lookup and
                   Assigned(Combo.ComboGrid.LookupDataSource) and
                   (Combo.ComboGrid.Datasource = Combo.ComboGrid.LookupDatasource) then
                begin
                    ...
                    Value := Combo.ComboGrid.ActiveCell[ValueCol, CurBkm]
            end
            else if ControlType = ctText then
                Value := CheckStrValue(ControlType, Value);
        end;
    finally
        Combo.ComboGrid.FGridData.ActiveRecord := OldActive;
    end;
end;
...
begin
    if not Combo.ComboGrid.DataBound then begin inherited; Exit; end;
    if not Combo.ComboGrid.Active then Exit;

    if (not Col[CurDataCol].Lookup) or
       (not Assigned(Combo.ComboGrid.LookupDatasource)) or
       (Combo.ComboGrid.Datasource <> Combo.ComboGrid.LookupDatasource) then
    begin
        inherited;
    end
...
begin
    ...
    with Combo.ComboGrid.FGridData do
    begin
...
begin
    if not Combo.ComboGrid.DataBound then begin inherited; Exit; end;
    if not Combo.ComboGrid.Active then Exit;
    if (Combo.ValueCol < 1) or (Combo.ValueCol > Combo.ComboGrid.Cols) then Exit;

    if Combo.AutoSearch <> asNone then
    begin
        Combo.ComboGrid.SaveDatasetPosition(OldDatasetPos);
        try
            Combo.ComboGrid.FGridData.CheckDisableControls;
            ...
            Combo.ComboGrid.RestoreDatasetPosition(OldDatasetPos, Reset);
        end;
    end;
end;
...
begin
    ...
    if Combo.ValueColSorted then
    begin
        if CurBkm = Combo.ComboGrid.FGridData.LastBookmark then
            NewBkm := FindDBComboRow(EmptyBookmark, EmptyBookmark, SearchValue, False, Found)
        else
        begin
            Combo.ComboGrid.FGridData.NextBookmark(CurBkm, NextBkm);
            ComboValue := Combo.ComboGrid.BookmarkValue(Combo.ValueCol, NextBkm);
            if (ComboValue = '') or not Combo.ComboValueEqual(ComboValue[1], SearchValue) then
                NewBkm := FindDBComboRow(EmptyBookmark, EmptyBookmark, SearchValue, False, Found)
            else
            begin
                NewBkm := NextBkm;
...
begin
    Found := False;
    CurValue := string(CurrentCell.Value);
    if (CurValue = '') or (not Combo.ComboValueEqual(CurValue[1], Key)) then
        SearchBkm := FindDBComboRow(EmptyBookmark, EmptyBookmark, Key, False, Found)
    else
    begin
        ComboValue := '';
        SearchBkm := Combo.ComboGrid.CurrentDataRow;
        if SearchBkm <> EmptyBookmark then
            ComboValue := Combo.ComboGrid.BookmarkValue(Combo.ValueCol, SearchBkm);

        if not Combo.ComboValueEqual(CurValue, ComboValue) then
        begin
            SearchBkm := FindDBComboRow(EmptyBookmark, EmptyBookmark, CurValue, False, Found);
            Found := False;
            ComboValue := '';
            if VarToStr(SearchBkm) <> VarToStr(EmptyBookmark) then
                ComboValue := Combo.ComboGrid.BookmarkValue(Combo.ValueCol, SearchBkm);
            if (ComboValue = '') or (not Combo.ComboValueEqual(ComboValue[1], Key))
    ...
    begin
        ...
        begin
            ...
            Found := (Length(SearchValue) <= Length(ComboValue)) and
                     (Combo.ComboValueEqual(Copy(ComboValue, 1, Length(SearchValue)), SearchValue));
        end;

        if Found then
            FComboSearchValue := SearchValue
        else if (Length(FComboSearchValue) = 1) and
                Combo.ComboValueEqual(FComboSearchValue[1], Key) then
        begin
...
var
    ...
    OldDatasetPos: TtsDatasetPosition;
begin
    if not Combo.ComboGrid.DataBound then
    begin
      Result := inherited Combo.PositionDropDownList(Key);
      Exit;
    end;
    if not Combo.ComboGrid.Active then
       exit;
    ...
    try
        ...
        begin
            SetDBComboRow(SearchBkm);
            DoComboGetValue(CurDataCol, CurrentDataRow, Combo.ComboGrid.CurrentDataRow, Result);
            ...
            begin
               ...
               begin
                 if VarIsEmpty(ComboValue) or VarIsNull(ComboValue) then
                    ...
                    Result := Unassigned
                 else if (not Combo.ValueColSorted) and (not Combo.ComboValueEqual(ComboValue[1], Key)) then
                    Result := Unassigned
                 else
                    Result := ComboValue;
               end;
...
begin
    ...
    try
        ...
        begin
            SetDBComboRow(SearchBkm);
            NewValue := Unassigned;
            DoComboGetValue(CurDataCol, CurrentDataRow, Combo.ComboGrid.CurrentDataRow, NewValue);
        ...
        Combo.ComboGrid.RestoreDatasetPosition(OldDatasetPos, not Result);
    end;
end; }
...
begin
    ...
    try
        ...
        Msg := '';
        if (Self.CompareBkmPos(FirstBkmPos, LastBkmPos) = 0) then
           Msg := ResourceStr(SDeleteRecordQuestion)
        else
           Msg := ResourceStr(SDeleteMultipleRecordsQuestion);
    finally
        EnablePaint := True;
...
begin
    ...
    begin
        ...
        //    TtsDBCombo(Element.FCombo).ComboGrid.ChangeScale(M, D);
    end;
end;
...
begin
   // if Assigned(FOnComboInit) then FOnComboInit(Self, Combo.ComboGrid, DataCol, DataRow);
end;

procedure TtsCustomDBGrid.DoComboDropDown(DataCol: Longint; DataRow: Variant);
begin
   // if Assigned(FOnComboDropDown) then FOnComboDropDown(Self, Combo.ComboGrid, DataCol, DataRow);
end;

procedure TtsCustomDBGrid.DoComboRollUp(DataCol: Longint; DataRow: Variant);
begin
  //  if Assigned(FOnComboRollUp) then FOnComboRollUp(Self, Combo.ComboGrid, DataCol, DataRow);
end;
...
begin
  ...
  //      FOnComboGetValue(Self, Combo.ComboGrid, GridDataCol, GridDataRow, ComboDataRow, Value);
end;
...
begin
  ...
  //      FOnComboCellLoaded(Self, Combo.ComboGrid, DataCol, DataRow, Value);
end;
